//! BOT SIM session API: acquire starts in-process bot_sim; release + grace stops it.
...
#[test]
fn bot_sim_session_starts_physics_and_stops_after_release() {
    ...
    let idle: serde_json::Value = ureq::get(&format!("{base}/api/v1/bot-sim"))
        .call()
        .expect("status idle")
        .into_json()
        .expect("json");
    ...
    let session: serde_json::Value = ureq::post(&format!("{base}/api/v1/bot-sim/session"))
        .call()
        .expect("acquire")
        .into_json()
        .expect("json");
    ...
    let mut viewer = Node::with_options("bot_sim_test_viewer", opts);
    let cmd = viewer
    ...
    ureq::delete(&format!("{base}/api/v1/bot-sim/session/{session_id}"))
        .call()
        .expect("release");

    // Default stop grace is 2s; sweep on status.
    thread::sleep(Duration::from_millis(2500));
    let after: serde_json::Value = ureq::get(&format!("{base}/api/v1/bot-sim"))
        .call()
        .expect("status after")
        .into_json()
        .expect("json");
