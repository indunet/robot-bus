...
fn localize_env_line(line: &str, lang: ReportLang) -> String {
    match lang {
        ReportLang::Zh => line.to_string(),
        ReportLang::En => line
            .replacen("系统:", "OS:", 1)
            .replacen("逻辑 CPU:", "Logical CPUs:", 1)
            .replacen("CPU 核心(物理):", "CPU cores (physical):", 1)
            .replacen("CPU 核心(逻辑):", "CPU cores (logical):", 1)
            .replacen("内存:", "Memory:", 1),
    }
}
...
fn render_perf_report(
    ...
    let trial_secs = env_f64("ROBOT_BUS_PERF_GOODPUT_TRIAL_SECS", 1.0);
    let max_loss = env_f64("ROBOT_BUS_PERF_MAX_LOSS_PCT", 1.0);
    let latency_samples = env_usize("ROBOT_BUS_PERF_MSG_LATENCY_SAMPLES", 5_000);
    let svc_iters = env_usize("ROBOT_BUS_PERF_SVC_ITERS", 10_000);
    let act_iters = env_usize("ROBOT_BUS_PERF_ACT_ITERS", 5_000);

    let mut md = String::new();
    match lang {
        ReportLang::Zh => {
            md.push_str("[English](../en/perf-report.md) | 中文\n\n");
            md.push_str("# robot-bus 性能测试报告\n\n");
            md.push_str("由 `just perf`（`cargo run --release --bin robot_bus_perf`）生成。\n\n");
            md.push_str("结果使用前请核对环境、缺测项与发送/接收统计窗口，见[性能报告说明](performance.md)。\n\n");
            md.push_str("## 环境\n\n");
        }
        ReportLang::En => {
            md.push_str("English | [中文](../zh/perf-report.md)\n\n");
            md.push_str("# robot-bus performance report\n\n");
            md.push_str(
                "Generated by `just perf` (`cargo run --release --bin robot_bus_perf`).\n\n",
            );
            md.push_str("Before using results, verify the environment, missing scenarios and send/receive counting windows; see [performance interpretation](performance.md).\n\n");
            md.push_str("## Environment\n\n");
        }
    }
    for line in env_lines {
        md.push_str("- ");
        md.push_str(&localize_env_line(line, lang));
        md.push('\n');
    }
    md.push('\n');
    ...
    match lang {
        ReportLang::Zh => {
            ...
            md.push_str(&format!(
                "- Message **吞吐（主指标）**：按目标速率限速发送约 {trial_secs:.1}s（可用 `ROBOT_BUS_PERF_GOODPUT_TRIAL_MSGS` 改为固定条数），**二分搜索**丢包率 ≤ {max_loss:.1}% 且发送窗口内 pub/sub 均 ≥90% 目标速率的最大可持续速率（max goodput）。\n"
            ));
            md.push_str(&format!(
                "- Message **延迟**：另做 {latency_samples} 次限速抽样（发一条等收到再发），测单程时延。\n"
            ));
            md.push_str(&format!(
        ...
        ReportLang::En => {
            ...
            md.push_str(&format!(
                "- Message **throughput (primary)**: paced send for ~{trial_secs:.1}s (or fixed count via `ROBOT_BUS_PERF_GOODPUT_TRIAL_MSGS`); **binary search** for max sustainable rate (max goodput) with loss ≤ {max_loss:.1}% and pub/sub ≥90% of the target rate in the send window.\n"
            ));
            md.push_str(&format!(
                "- Message **latency**: separate paced sample of {latency_samples} (send one, wait for receive).\n"
            ));
            md.push_str(&format!(
...
/// Writes Chinese and English reports; returns the Chinese path (primary).
pub fn write_report(results: &[ScenarioResult], env_lines: &[String]) -> std::io::Result<PathBuf> {
    let root = repo_root();
    let zh_path = root.join("docs/zh/perf-report.md");
    let en_path = root.join("docs/en/perf-report.md");
    ...
    fs::write(
        &zh_path,
        render_perf_report(results, env_lines, ReportLang::Zh),
    )?;
    fs::write(
        &en_path,
        render_perf_report(results, env_lines, ReportLang::En),
    )?;
    Ok(zh_path)
}
