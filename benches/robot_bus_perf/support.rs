...
fn render_perf_report(
    ...
    match lang {
        ...
        ReportLang::En => {
            ...
            md.push_str("Generated by `just perf` (`cargo run --release --bin robot_bus_perf`).\n\n");
...
pub fn write_report(results: &[ScenarioResult], env_lines: &[String]) -> std::io::Result<PathBuf> {
    ...
    fs::write(&zh_path, render_perf_report(results, env_lines, ReportLang::Zh))?;
    fs::write(&en_path, render_perf_report(results, env_lines, ReportLang::En))?;
    Ok(zh_path)
}
