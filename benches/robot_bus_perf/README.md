...
robot-bus 本机性能压测：进程内 `RobotBusBroker`，覆盖 **tcp / ipc / inproc / ws** 的 pub/sub、service、action。
...
- **Action**：ws 场景名为 `action SendGoal`，对应一元 goal 请求 + server-stream `FEEDBACK` / `RESULT`
