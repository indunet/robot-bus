...
fn main() {
    ...
    // ROS 2 bridge vendors `std_srvs` typesupport; search paths come from a
    // sourced workspace (AMENT_PREFIX_PATH), same as rclrs.
    if env::var_os("CARGO_FEATURE_ROS2").is_some() {
        ...
        println!("cargo:rustc-link-lib=dylib=std_srvs__rosidl_typesupport_c");
        println!("cargo:rustc-link-lib=dylib=std_srvs__rosidl_generator_c");
    }
