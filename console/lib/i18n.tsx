...
const en = {
  ...
  queueTotal: '{n} overflow drops since server start',
  queueScope: 'Pending at the server only; excludes network and client queues. Each row is one subscription filter.',
  queueFilter: 'Filter',
  queuePolicy: 'Overflow policy',
  queuePending: 'Pending / capacity',
  queueDropped: 'Overflow drops',
  queueDropNewest: 'Discard incoming',
  queueKeepLast: 'KeepLast({n})',
  ...
  bridgesTitle: 'ROS2 BRIDGE',
  bridgesActive: '{n} bridges · {r} routes',
  bridgesEmpty: 'No Ros2Bridge snapshot yet',
  colKind: 'KIND',
  colDirection: 'DIR',
  colRos: 'ROS',
  colBus: 'BUS',
  ...
  colIdle: 'IDLE',
  bridgeIdle: 'No data',
  bridgeStalled: 'Stalled',
  bridgeErrorCount: 'DROP/FAIL',
  bridgeState: 'STATUS',
  bridgeSucceeded: 'Succeeded',
  bridgeFailed: 'Failed',
  bridgeAborted: 'Aborted',
  bridgeTimeout: 'Timeout',
  bridgeCancelled: 'Cancelled',
  bridgeRejected: 'Rejected',
  bridgePending: 'No calls',
  bridgeRunning: 'Running',
  bridgeRpcCounts: 'Calls {calls} · Succeeded {success} · Active {active} · Failed {failures} (timeouts {timeouts}, rejected {rejected}) · Cancelled {cancelled}',
  bridgeLastError: 'Last error',
  bridgeRpcTraffic: 'Calls / succeeded; counters describe bridge and transport outcomes',
  bridgeLazy: 'lazy',
  bridgedMark: 'bridged',
...
const zh: Record<MessageKey, string> = {
  queueTitle: 'WS 订阅队列',
  queueTotal: '服务端启动以来溢出丢弃 {n} 条',
  queueScope: '仅统计服务端待发送队列，不含网络和客户端积压。每行对应一个订阅过滤器。',
  queueFilter: '订阅过滤器',
  queuePolicy: '积压策略',
  queuePending: '待发送 / 容量',
  queueDropped: '溢出丢弃',
  queueDropNewest: '丢弃新到消息',
  queueKeepLast: 'KeepLast({n}) · 保留最新 {n} 条',
  ...
  bridgesTitle: 'ROS2 桥',
  bridgesActive: '{n} 座桥 · {r} 条路由',
  bridgesEmpty: '尚无 Ros2Bridge 快照',
  colKind: '类型',
  colDirection: '方向',
  colRos: 'ROS',
  colBus: '总线',
  ...
  bridgePending: '尚无调用',
  bridgeRunning: '执行中',
  bridgeRpcCounts: '调用 {calls} · 成功 {success} · 进行中 {active} · 失败 {failures}（超时 {timeouts}、拒绝 {rejected}）· 取消 {cancelled}',
  bridgeLastError: '最近错误',
  bridgeRpcTraffic: '调用数 / 成功数；统计桥接及通信结果',
  bridgeLazy: '惰性',
  bridgedMark: '已桥接',
