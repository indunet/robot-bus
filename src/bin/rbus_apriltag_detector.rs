//! CLI: Image → AprilTagDetectionArray (feature `apriltag-detector`).

use anyhow::{Context, Result};
use clap::Parser;
use robot_bus::apriltag_detector::{run, EXAMPLE_CONFIG};
use robot_bus::NodeOptions;

#[derive(Debug, Parser)]
#[command(
    name = "rbus_apriltag_detector",
    about = "Subscribe to sensor_msgs/Image and publish apriltag_msgs/AprilTagDetectionArray"
)]
struct Args {
    /// Node name on the bus.
    #[arg(long, default_value = "apriltag_detector")]
    name: String,
...
fn main() -> Result<()> {
    ...
    run(&args.name, options, args.params.as_deref())
        .with_context(|| format!("run apriltag detector node {}", args.name))?;
    Ok(())
}
