...
| --- | --- | --- |
| Core SDK | `--no-default-features` | Native ZMQ SDK and broker primitives |
| WS gateway | `--no-default-features --features ws` | Core + WS clients/gateway, discovery and subscription queue API |
| Monitoring API | `--no-default-features --features ws,console-api` | Gateway + monitoring REST API and bus control plane |
| Embedded console | `--no-default-features --features ws,console` | Monitoring + embedded web UI, without tank simulation |
| Full (default) | no feature flags, or `--features full` | Gateway + console + tank demo |
...
From a source checkout, run `just gen-rust` once to generate protobuf stubs. Only builds with `console` require `just console` (Node/npm/pnpm and TypeScript code generation). Core, gateway, and monitoring builds need no web assets or frontend build tools. Native ZeroMQ build requirements still apply. Published Rust packages include generated stubs.

Convenience commands: `just build-sdk`, `just build-gateway`, `just build-monitoring`, and `just build-full`. These produce release builds; the SDK command builds the library, and the others build `robot_bus_broker`.
