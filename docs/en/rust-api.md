...
# WebSocket RPC gateway (`ws` feature) is on by default; to disable: robot-bus = { version = "2.3.0", default-features = false }
```
...
node.create_subscription_raw(
    "/robot1/imu",
    Arc::new(|_| { /* may run in parallel with other callbacks in the same group */ }),
    Some(&reentrant),
)?;
node.create_timer(
    Duration::from_millis(100),
