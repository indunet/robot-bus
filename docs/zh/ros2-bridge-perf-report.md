...
- 运行 `just perf-ros2-bridge`（`ros2` 容器 + rust IDL overlay）后回填。

## 方法

- 进程内 broker + `Ros2Bridge`；ROS 与 bus 各一条 peer。
- 场景：64B `std_msgs/String`；Image 默认 640×480 rgb8（`ROS2_BRIDGE_PERF_IMAGE_WIDTH/HEIGHT` 可改）。
- 方向：ROS→bus 与 bus→ROS。
