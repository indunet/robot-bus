...
- C++: bindings/cpp/include/robot_bus/ros2_bridge/mappers/<pkg>/<msg>.hpp
  and umbrella bindings/cpp/include/robot_bus/ros2_bridge_topic_mappers.hpp
...
CPP_UMBRELLA = (
    ROOT / "bindings" / "cpp" / "include" / "robot_bus" / "ros2_bridge_topic_mappers.hpp"
)
...
def cpp_to_ros_stmt(msg: Msg, f: Field, existing: dict[str, tuple[Path, str]]) -> str:
    ...
    if f.is_bytes:
        return (
            f"  ::robot_bus::ros2_bridge_mappers::copy_seq(out.{f.name}, {bus});"
        )
    if f.is_string and f.repeated:
        return (
            f"  out.{f.name}.clear();\n"
...
def write_cpp_umbrella(headers: list[tuple[str, str]]) -> None:
    """Write one include-all header so ros2_bridge.hpp sees generated mappers."""
    lines = [
        "#pragma once",
        "",
    ...
    for pkg, stem in sorted(headers):
        lines.append(f"#include <robot_bus/ros2_bridge/mappers/{pkg}/{stem}.hpp>")
    lines.append("")
    CPP_UMBRELLA.write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    cpp_headers: list[tuple[str, str]] = []
    py_written = 0
    cpp_written = 0
    for path, struct, ros_type in mappers:
        msg = all_msgs[ros_type]
        ...
        cpp_headers.append((pkg, stem))
        cpp_written += 1

    write_python_init(py_generated)
    write_cpp_umbrella(cpp_headers)
    print(
        f"wrote {written} rust mappers (skipped {skipped} gold), "
        f"{py_written} python, {cpp_written} cpp, umbrella {CPP_UMBRELLA.name}, "
        f"{SHIM_OUT}"
    )
