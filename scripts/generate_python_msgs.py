...
def write_leaf_reexports(pkg_dir: Path) -> None:
    """For each msg/v1 or srv/v1 dir, re-export message classes from *_pb2."""
    for leaf in list(pkg_dir.rglob("msg")) + list(pkg_dir.rglob("srv")):
        v1 = leaf / "v1"
        if not v1.is_dir():
            continue
...
def write_pkg_root_init(pkg: str) -> None:
    ...
    path.write_text(
        ...
        f'"""ROS-style protobuf types: robot_bus.{pkg}.{{msg|srv}}.v1"""\n',
        encoding="utf-8",
    )
